...
use uacpi::sys::{
    acpi_entry_hdr, acpi_madt_interrupt_source_override, acpi_madt_ioapic,
    ACPI_MADT_ENTRY_TYPE_INTERRUPT_SOURCE_OVERRIDE, ACPI_MADT_ENTRY_TYPE_IOAPIC,
};
use uacpi::{table_find_by_signature, MadtIoapic, MadtIrqSourceOverride, MADT_SIGNATURE};
extern crate alloc;
use alloc::vec::Vec;
#[derive(Debug)]
struct ioapic {
...
impl ioapic {
    ...
    fn select_reg(&self, reg: u8) {
        unsafe {
            *((self.addr as u64 + HDDM_OFFSET.get_response().unwrap().offset()) as *mut u32) =
                reg as u32;
        }
    }
    fn write_toreg(&self, val: u32) {
        unsafe {
            *((self.addr as u64 + 0x10 + HDDM_OFFSET.get_response().unwrap().offset())
                as *mut u32) = val;
        }
    }
    fn read_reg(&self) -> u32 {
        unsafe {
            return *((self.addr as u64 + 0x10 + HDDM_OFFSET.get_response().unwrap().offset())
                as *mut u32);
        }
    }
...
pub fn apic_init() {
    let mut table: *mut uacpi::Madt = table_find_by_signature(MADT_SIGNATURE)
        .unwrap()
        .get_virt_addr() as *mut uacpi::Madt;
    unsafe {
        apicdriv = Some(SystemAPIC {
            apics: Vec::new(),
            sourceoveride: HashMap::new(),
        })
    };
    println!("got table");
    unsafe {
        ...
        while offset < length_of_entries {
            let entry_hdr = ((*table)
                .entries
                .as_mut_ptr()
                .add(offset / size_of::<acpi_entry_hdr>()));
            ...
            match (*entry_hdr).type_ as u32 {
                ACPI_MADT_ENTRY_TYPE_INTERRUPT_SOURCE_OVERRIDE => {
                    ...
                    apicdriv
                        ...
                        .unwrap()
                        .sourceoveride
                        .insert(real.source as usize, real);
                }
