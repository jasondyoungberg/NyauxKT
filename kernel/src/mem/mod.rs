...
unsafe impl GlobalAlloc for MemoryManager
{
    unsafe fn alloc(&self, layout: core::alloc::Layout) -> *mut u8 {
        if (layout.size() > 4096)
        {
            return cur_pagemap.as_mut().unwrap().vmm_region_alloc(layout.size() as u64, VMMFlags::KTPRESENT.bits() | VMMFlags::KTWRITEALLOWED.bits()).unwrap();
        }
        else {
            return kmalloc_manager.as_mut().unwrap().alloc(layout.size()).unwrap();
        }
    }
    ...
    unsafe fn realloc(&self, ptr: *mut u8, layout: core::alloc::Layout, new_size: usize) -> *mut u8 {
        if (layout.size() > 4096)
        {
            let mut new = cur_pagemap.as_mut().unwrap().vmm_region_alloc(new_size as u64, VMMFlags::KTPRESENT.bits() | VMMFlags::KTWRITEALLOWED.bits()).unwrap();
