...
impl PageMap {
    ...
    pub fn map(&mut self, from_virt: u64, to_phys: u64, flags: u64) -> Result<(), &'static str> {
        ...
        let mut cur_table =
            (self.rootpagetable as u64 + HDDM_OFFSET.get_response().unwrap().offset()) as *mut u64;
        cur_table = PageMap::get_next_table(cur_table, lvl4table_index, true, Some(flags));
        cur_table = PageMap::get_next_table(
            (cur_table as u64 + HDDM_OFFSET.get_response().unwrap().offset()) as *mut u64,
            lvl3table_index,
            true,
            Some(flags),
        );
        ...
        unsafe {
            (*((cur_table as u64 + HDDM_OFFSET.get_response().unwrap().offset()) as *mut u64)
                .offset(lvl1table_index as isize)) = entry;
        }
    ...
    pub fn virt_to_phys(&mut self, from_virt: u64) -> Result<u64, &'static str> {
        ...
        unsafe {
            if *((cur_table as u64 + HDDM_OFFSET.get_response().unwrap().offset()) as *mut u64)
                .offset(lvl1table_index as isize)
                != 0
            {
                return Ok(PageMap::get_phys_from_entry(
                    *((cur_table as u64 + HDDM_OFFSET.get_response().unwrap().offset())
                        as *mut u64)
                        .offset(lvl1table_index as isize),
                ));
            } else {
            ...
            unsafe {
                if (*cur_node.unwrap()).base == addr {
                    ...
                    for i in 0..num_of_pages {
                        ...
                        self.unmap((*cur_node.unwrap()).base + (i * 0x1000), 0)
                            .unwrap();
                        phys =
                            Ok(phys.unwrap() as u64 + HDDM_OFFSET.get_response().unwrap().offset());
    ...
    pub fn vmm_region_alloc_non_backing(
        &mut self,
        phys_addr: u64,
        size: u64,
        flags: u64,
    ...
    pub fn new_inital() {
        ...
        q.rootpagetable =
            (q.rootpagetable as u64 + HDDM_OFFSET.get_response().unwrap().offset()) as *mut u64;
        ...
        println!(
            "Kernel Location: phys: {:#x} virt: {:#x}",
            ADDR.get_response().unwrap().physical_base(),
            ADDR.get_response().unwrap().virtual_base()
        );
            ...
            match i.entry_type {
                ...
                | EntryType::RESERVED => {
                    if i.entry_type == EntryType::RESERVED {
                        i.base = (super::phys::align_down(i.base as usize, 4096)
                            + i.base as usize % 4096) as u64;
                    }
