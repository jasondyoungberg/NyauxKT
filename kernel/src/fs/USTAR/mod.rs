...
use crate::{fs::{tmpfs::tmpfsdir, vfs::{get_list, vnode}}, mem::phys::{align_up, RAMUSAGE}, println, utils::get_limine_file};
extern crate alloc;
use alloc::string::*;
    ...
    if !cfg!(target_os = "linux") {
        ...
        while (e as u64) < ((o.as_mut_ptr() as u64 + o.len() as u64) - 1024) {
            unsafe {
                match (*e).typeflag
                {
                    b'0' => {
                        ...
                        std::println!("creating file with path {}", get_list(okkkk).last().unwrap());
                        let ok = letskeepitnice.create(get_list(okkkk).last().unwrap()).unwrap();
                        let mut v = [0; 12];
            ...
            unsafe {
                let mut v = [0; 12];
                v.copy_from_slice(&(*e).filesize);
                std::println!("{}", oct2bin(CStr::from_bytes_until_nul(&v).unwrap().to_str().unwrap()));
                
                
                e = (e as u64 + 512 + align_up( oct2bin(CStr::from_bytes_until_nul(&v).unwrap().to_str().unwrap()) as usize, 512) as u64) as *mut UStarHeader;
            }
...
pub fn ustarinit()
{
    ...
    unsafe {
        ...
        CUR_VFS.as_mut().unwrap().vnode = Some(
            Rc::new(RefCell::new(tmpfsdir::default()))
        )
    }
            ...
            unsafe {
                match (*e).typeflag
                {
                    b'0' => {
                        let okkkk = CStr::from_bytes_until_nul(&(*e).name).unwrap().to_str().unwrap();
                        println!("file found");
                        println!("creating file with name {}", get_list(okkkk).last().unwrap());
                    ...
                    b'5' => {
                        println!("directory");
                        let okkkk = CStr::from_bytes_until_nul(&(*e).name).unwrap().to_str().unwrap();
                        println!("creating directory with name {}", get_list(okkkk).last().unwrap());
                        for i in get_list(okkkk) {
                            let res = cur_dir.as_mut().unwrap().borrow_mut().lookup(i);
                            if let Ok(ress) = res {
                                *cur_dir = Some(ress);
